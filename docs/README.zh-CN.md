...
如果希望自行决定何时删除 Cookie，请使用用户脚本。它不会自动删除 Cookie。在 Chromium 中请优先使用上述扩展程序；只有必须手动清理时才使用 Tampermonkey Beta。

Violentmonkey 2.43.x 和 2.44.0 会把 Firefox 专用的 `firstPartyDomain` 添加到 Chromium Cookie API 请求中，因此无法在 Chromium 150 中枚举 Cookie。用户脚本无法删除管理器 Service Worker 内部添加的字段。请改用 Chromium 扩展程序或 Tampermonkey Beta。

1. 在 Firefox 中安装 [Violentmonkey](https://violentmonkey.github.io/)。在 Chromium 中，仅当不使用扩展程序时才安装 Tampermonkey Beta。
...
目标 Cookie 为 HttpOnly。在 Firefox 中使用 Violentmonkey 时，请同时启用以下两项设置：
...
截至 2026 年 7 月，Firefox 推荐使用 Violentmonkey。Chromium 推荐使用浏览器扩展程序；Tampermonkey Beta 也支持手动清理，但不在本项目的常规测试范围内。Tampermonkey 稳定版和 FireMonkey 无法访问目标 HttpOnly Cookie。
