...
パッケージ化されていない拡張機能とユーザースクリプトは自動更新されません。新しいリリースは手動でインストールしてください。
...
拡張機能はインストール直後に一時チャットのタブが開いているか確認し、開いていなければ対象 Cookie をすぐに削除します。クリーンアップに成功すると3時間後に確認し、その時点で一時チャットのタブが開いている場合は削除せず、タブが閉じるまで30分ごとに確認します。ブラウザーを完全に終了してから起動した場合も、対象 Cookie をすぐに削除します。ツールバーボタンをクリックすると、まず開いている一時チャットのタブを確認し、なければ対象 Cookie をすぐに削除して、削除した件数をバッジに表示します。一時チャットのタブが開いている場合は何も削除しません。
...
現在、Violentmonkey は Firefox 専用の `firstPartyDomain` を Chromium の Cookie API リクエストに追加するため、Chromium 150 では Cookie を列挙できません。

1. Firefox では [Violentmonkey](https://violentmonkey.github.io/) または [Tampermonkey Beta](https://www.tampermonkey.net/index.php?browser=firefox) をインストールします。Chromium では、拡張機能を使わない場合に [Tampermonkey Beta](https://www.tampermonkey.net/index.php?browser=chrome) をインストールします。
2. Chromium 138 以降では、ユーザースクリプトマネージャー拡張機能の詳細を開き、**Allow User Scripts** を有効にします。
3. リリースの添付ファイルから `chatgpt-http-error-431-fixer.user.js` をダウンロードします。
4. ダウンロードしたファイルをユーザースクリプトマネージャーにインポートするか、新しいスクリプトを作成してファイルの内容を貼り付けます。
...
対象 Cookie は HttpOnly のため、Tampermonkey は Beta 版を使用する必要があります。Firefox で Violentmonkey を使う場合は、次の2つの設定を有効にしてください。
...
2026年7月現在、Firefox では Violentmonkey または Tampermonkey Beta を使用できますが、どちらの Firefox 環境もこのプロジェクトでは直接テストしていません。Chromium ではブラウザー拡張機能を推奨し、ユーザースクリプトが必要な場合は Tampermonkey Beta を使用してください。Tampermonkey の安定版と FireMonkey は、対象の HttpOnly Cookie にアクセスできません。

問題が発生した場合は、[GitHub Issues](https://github.com/ilsd7/chatgpt-http-error-431-fixer/issues) でお知らせください。
...
拡張機能は通常の Cookie とパーティション Cookie の両方を確認します。ユーザースクリプトは通常の Cookie と ChatGPT パーティションを照会し、両方の照会に成功した場合にのみ削除を開始します。パーティション照会だけが利用できない場合、**Count** は通常の Cookie だけを数えた結果であることを明示します。ログイン Cookie や、ほかの ChatGPT Cookie は削除対象ではありません。
