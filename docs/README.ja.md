...
削除するタイミングを自分で決めたい場合はユーザースクリプトを使います。Cookie を自動的に削除することはありません。Chromium では上記の拡張機能を優先し、手動クリーンアップが必要な場合にのみ Tampermonkey Beta を使ってください。

Violentmonkey 2.43.x と 2.44.0 は Firefox 専用の `firstPartyDomain` を Chromium の Cookie API リクエストに追加するため、Chromium 150 では Cookie を列挙できません。ユーザースクリプトから、マネージャーの Service Worker が追加したフィールドを取り除くことはできません。Chromium 拡張機能または Tampermonkey Beta を使用してください。

1. Firefox では [Violentmonkey](https://violentmonkey.github.io/) をインストールします。Chromium では、拡張機能を使わない場合にのみ Tampermonkey Beta をインストールします。
...
対象 Cookie は HttpOnly です。Firefox で Violentmonkey を使う場合は、次の2つの設定を有効にしてください。
...
2026年7月現在、Firefox では Violentmonkey を推奨します。Chromium ではブラウザー拡張機能を推奨します。Tampermonkey Beta も手動クリーンアップに対応しますが、このプロジェクトでは定期的にテストしていません。Tampermonkey の安定版と FireMonkey は、対象の HttpOnly Cookie にアクセスできません。
