...
通过安全清理累积的临时聊天 Cookie，防止 ChatGPT 反复出现 HTTP 431 错误。

使用临时聊天时，系统会不断创建一个月后过期的 `conv_key_*` Cookie。频繁使用后，这些 Cookie 可能累积到数十个甚至更多，使请求头过大，并导致 **HTTP 431: Request Header Fields Too Large** 错误。本项目只删除这些累积的 Cookie，不会影响登录 Cookie 或其他 ChatGPT Cookie。
...
2. 从 GitHub Release 附件中下载 `chatgpt-http-error-431-fixer.user.js`。
3. 将下载的文件导入用户脚本管理器，或者新建脚本并粘贴文件内容。
...
本工具并不能解决所有 HTTP 431 错误，仅适用于因临时聊天 Cookie 累积而导致的错误。
